...
if __name__=='__main__':
    ...
    tree = GitlabTree(arguments["<gitlab-url>"], arguments["--token"], arguments["--include"], arguments["--exclude"], arguments["--concurrency"], in_file=in_file)
    log.debug("Reading projects tree from gitlab at [%s]", arguments["<gitlab-url>"])
    tree.load_tree()
    if tree.is_empty():
